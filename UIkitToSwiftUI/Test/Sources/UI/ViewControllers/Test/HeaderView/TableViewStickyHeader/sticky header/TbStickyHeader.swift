...
public class TbStickyHeader: NSObject {
    
    /**
     The view containing the provided header.
     */
    private(set) lazy var contentView: TbStickyHeaderView = {
        let view = TbStickyHeaderView()
        view.parent = self
    ...
    public override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        if let path = keyPath, context == &TbStickyHeaderView.KVOContext && path == "contentOffset" {
            self.layoutContentView()
        } else {
